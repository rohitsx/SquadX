...
const RegisterPage: React.FC = () => {
  ...
  const [formData, setFormData] = useState({ username: '', email: '', dob: '', password: '' });
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-black text-white">
      ...
      <div className="z-10 w-full max-w-md p-8 space-y-6 bg-black bg-opacity-50 rounded-xl backdrop-blur-md">
        ...
        <h1 className="text-2xl font-bold uppercase text-white text-center">Register</h1>

        <form onSubmit={handleSubmit} className="space-y-4">
          {['username', 'email', 'dob', 'password'].map((field) => (
            <div key={field}>
              <label htmlFor={field} className="text-sm font-medium block mb-1 text-neutral-300">
                {field.charAt(0).toUpperCase() + field.slice(1)}
              </label>
              <div className="relative">
                <input
                  id={field}
                  type={field === 'password' ? (showPassword ? 'text' : 'password') : field === 'dob' ? 'date' : 'text'}
